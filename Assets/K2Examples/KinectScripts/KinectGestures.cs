...
//			return new int[0];
	...
	public virtual void CheckForGesture(long userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		switch (gestureData.gesture)
		{
			...
			case Gestures.Gesture1:
				switch (gestureData.state)
				{
					case 0:// gesture detection
						if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
						jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
						Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[rightHandIndex].z) < 0.3f &&
						Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[leftElbowIndex].y) < 0.3f &&
						(jointsPos[leftElbowIndex].z - jointsPos[leftHandIndex].z) < 0f && (jointsPos[rightElbowIndex].z - jointsPos[rightHandIndex].z) > 0f &&
						(jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z) > 0f)
					...
					case 1:
						bool isInPose = 
						jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
						jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
						Mathf.Abs(jointsPos[leftHandIndex].z - jointsPos[rightHandIndex].z) < 0.3f &&
						Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[leftElbowIndex].y) < 0.3f &&
						(jointsPos[leftElbowIndex].z - jointsPos[leftHandIndex].z) < 0f && (jointsPos[rightElbowIndex].z - jointsPos[rightHandIndex].z) > 0f &&
			...
			case Gestures.Gesture2:
				switch (gestureData.state)
				{
					case 0:
						if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
						jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
						(jointsPos[leftHandIndex].y - jointsPos[leftElbowIndex].y) > 0.1f && (jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) > 0.1f &&
						(jointsPos[leftElbowIndex].y - jointsPos[leftShoulderIndex].y) > 0.1f && (jointsPos[rightElbowIndex].y - jointsPos[rightShoulderIndex].y) > 0.1f &&
				...
				break;
			case Gestures.Gesture3:
                switch (gestureData.state)
                {
                    case 0:// gesture detection
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                        jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
						Mathf.Abs(jointsPos[leftElbowIndex].y - jointsPos[leftHandIndex].y) < 0.1f &&
						(jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) < 0.3f &&
                        Mathf.Abs(jointsPos[rightElbowIndex].y - jointsPos[rightHandIndex].y) < 0.1f &&
                        (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) < 0.3f )
            ...
            case Gestures.Gesture4:
				switch (gestureData.state)
				{
					case 0:
					{
                            if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                            jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
                            (jointsPos[leftHandIndex].z - jointsPos[leftElbowIndex].z) < 0f &&
                            (jointsPos[leftShoulderIndex].z - jointsPos[leftElbowIndex].z) < 0.6f &&
                            (jointsPos[leftShoulderIndex].z - jointsPos[leftElbowIndex].z) > 0.1f &&
                            (jointsPos[rightElbowIndex].z - jointsPos[rightShoulderIndex].z) > 0f &&
                            (jointsPos[rightHandIndex].z - jointsPos[rightElbowIndex].z) < 0.6f &&
                            (jointsPos[rightHandIndex].z - jointsPos[rightElbowIndex].z) > 0.1f &&
							(jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y)>0f &&
							(jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y)>0f
                            )
					...
					case 1:
					{
							...
							(jointsPos[leftElbowIndex].z - jointsPos[leftShoulderIndex].z) < 0.6f &&
							(jointsPos[leftElbowIndex].z - jointsPos[leftShoulderIndex].z) > 0.1f &&
							(jointsPos[rightShoulderIndex].z - jointsPos[rightElbowIndex].z) > 0.1f &&
							(jointsPos[rightElbowIndex].z - jointsPos[rightHandIndex].z) < 0.6f &&
							(jointsPos[rightElbowIndex].z - jointsPos[rightHandIndex].z) > 0.1f &&
							(jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) > 0f &&
							(jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) > 0f;
                ...
                break;
			case Gestures.Gesture5:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] &&
                        jointsTracked[leftShoulderIndex] && jointsTracked[leftElbowIndex] && jointsTracked[rightElbowIndex] &&
						(jointsPos[leftElbowIndex].y - jointsPos[leftHandIndex].y) > 0.1f && (jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) > 0.1f &&
						(jointsPos[rightElbowIndex].y - jointsPos[rightElbowIndex].y) > 0.1f && (jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) > 0.1f &&
						jointsPos[leftElbowIndex].z > jointsPos[leftHandIndex].z && MathF.Abs(jointsPos[leftHandIndex].z - jointsPos[leftShoulderIndex].z) < 0.3f &&
						jointsPos[rightShoulderIndex].z > jointsPos[rightElbowIndex].z && MathF.Abs(jointsPos[rightHandIndex].z - jointsPos[rightShoulderIndex].z) < 0.3f 
                        )
                        {
                            SetGestureJoint(ref gestureData, timestamp, leftElbowIndex, jointsPos[leftElbowIndex]);
                        }
